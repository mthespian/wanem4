   ...
   bridge*)
		...
		add)
			...
			then
				...
				# Sort the list. This feature is disabled
				#sortedNICList=`echo $nicList | sed 's/ /\\n/g' | sort -u`
				#Get the first NIC from the list
				firstNIC=`echo $nicList | awk '{ print $1 } '`	

				# Get the eth0 IP address for the bridge
				bridgeIP=`/sbin/ifconfig $firstNIC | grep inet | grep -v inet6 | awk '{ print $2 }' | awk -F: '{ print $2 }'`
				netmaskIP=`/sbin/ifconfig $firstNIC | grep inet | grep -v inet6 | awk '{ print $4 }' | awk -F: '{ print $2 }'`
				gatewayIP=`/sbin/route -n | grep UG | grep $firstNIC | awk '{ print $2 }'`
